...
/**
 * @requires ../externals/gxp/plugins/WMSCSource.js
 */
 ...
 *
 */
gdxp.WMSCSource = Ext.extend(gxp.plugins.WMSCSource, {
    
    /** api: ptype = gxp_wmscsource */
    ptype: "gdxp_wmscsource",
            ...
            for (var i=0, len=tileSets.length; i<len; i++) {
                ...
                if (tileSet.layers === layer.params.LAYERS && 
                    srs === this.getMapProjection().getCode()) {
                        var bbox = this.bbox || tileSet.bbox[srs].bbox;
                        ...
                        layer.params.TILED = config.tiled || true;
                        break;
                }
            }
        }
...
Ext.preg(gdxp.WMSCSource.prototype.ptype, gdxp.WMSCSource);
